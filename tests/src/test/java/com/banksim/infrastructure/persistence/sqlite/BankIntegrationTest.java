...
import com.banksim.domain.model.*;
import com.banksim.domain.repository.*;
...
class BankIntegrationTest {
    ...
    void testTransactionHistory() {
        ...
        // История по счёту-отправителю
        List<Transaction> historyFrom = bankService.getAccountHistory("ACC-010");
        assertEquals(3, historyFrom.size());

        // История по счёту-получателю
        List<Transaction> historyTo = bankService.getAccountHistory("ACC-020");
        assertEquals(3, historyTo.size());

        // Балансы после трёх переводов
    ...
    @Test
    @DisplayName("Интеграция: регистрация и аутентификация")
    ...
    @Test
    @DisplayName("Интеграция: перевод с несуществующего счёта — ошибка")
    ...
    @Test
    @DisplayName("Интеграция: перевод с закрытого счёта — ошибка")
    void testTransferFromClosedAccount() {
        ...
        // Перевод c закрытого счёта должен упасть
