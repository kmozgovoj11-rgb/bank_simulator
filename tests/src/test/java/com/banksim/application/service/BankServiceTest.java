...
class BankServiceTest {
    ...
    void setUp() {
        ...
        fromAccount = new DebitAccount(
                "acc-1", "ACC-001",
                new BigDecimal("200.00"), "USD",
                AccountStatus.ACTIVE, testCustomer);

        toAccount = new DebitAccount(
                "acc-2", "ACC-002",
    ...
    void testCreateCustomerEmptyName() {
        assertThrows(IllegalArgumentException.class, () -> {
            bankService.createCustomer("cust-1", "", "+79000000001");
        });
    ...
    void testTransferMoney() {
        ...
        when(accountRepository.findByNumber("ACC-002"))
                .thenReturn(Optional.of(toAccount));

        TransferTransaction result = bankService.transferMoney(
                "ACC-001", "ACC-002",
