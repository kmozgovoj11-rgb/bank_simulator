...
class DebitAccountTest {
    ...
    private Customer createTestCustomer() {
        Customer customer = new Customer("cust-1", "Alice", "+79000000001");
        return customer;
    }
    ...
    @Test
    @DisplayName("Проверка создания дебетового счёта")
    ...
    @Test
    @DisplayName("Нельзя снять больше, чем есть на счету")
    ...
    void testCloseAccount() {
        DebitAccount account = createTestAccount();
        account.withdraw(new BigDecimal("100.00")); // обнуляем баланс
    ...
    @Test
    @DisplayName("Нельзя закрыть счёт с ненулевым балансом")
